...
**Services:** PostgreSQL (`127.0.0.1:5432`) and Redis (`127.0.0.1:6379`) on a Docker bridge network (`minibot-net`). Both are localhost-only.
...
- Required secrets: `POSTGRES_PASSWORD`, `REDIS_PASSWORD` (additional API keys are commented placeholders in `minibot-secrets.sh`)
